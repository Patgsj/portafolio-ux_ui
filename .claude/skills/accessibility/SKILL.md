...
metadata:
  ...
  version: "1.1"
source: https://github.com/thedesignproject/agent-skills (skills/accessibility)
---
...
Interactive targets must be at least **24 x 24 CSS pixels** (AA). Exceptions: inline text links, elements where the browser controls the size, and targets where a 24px circle centered on the bounding box does not overlap another target.
...
/* Comfortable target size (recommended 44x44) */
.touch-target {
  min-width: 44px;
  min-height: 44px;
  display: inline-flex;
...
If a help mechanism (contact info, chat widget, FAQ link, self-help option) is repeated across multiple pages, it must appear in the **same relative order** each time.
...
Login flows must not rely on cognitive function tests unless a copy-paste/autofill mechanism, alternative method (passkey, SSO, email link), or object-recognition test is available.
